...
public class CreateClientController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        pAddress_box.selectedProperty().addListener((observableValue, oldVal, newVal) -> {
            //if checkbox has been checke
            if(newVal) {
                payeeAdress=createPayeeAdress();
    ...
    private void createClient() {
        ...
        Model.getInstance().getDatabaseDriver().createClient(fName,lName, payeeAdress, password, LocalDate.now());
    ...
    private void createAccount(String accountType) {
        ...
        //Create the checking account and/or savings account
        if(accountType.equals("Checking")) {
            Model.getInstance().getDatabaseDriver().createCheckingAccount(payeeAdress, accountNumber, 10, balance);
        } else {
            Model.getInstance().getDatabaseDriver().createSavingAccount(payeeAdress, accountNumber, 2000, balance);
        }
    }

    private void onCreatePayeeAddress() {
        if(fName_fld.getText()!=null & lName_fld.getText()!=null) {
            pAddress_lbl.setText(payeeAdress);
        }
    }

    private String createPayeeAdress() {
